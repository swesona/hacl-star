...
FStar_Dyn_dyn EverCrypt___proj__AES128_OPENSSL__item__st(EverCrypt_aes128_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES128_OPENSSL)
  {
    return projectee.val.case_AES128_OPENSSL;
  }
  else
  {
...
FStar_Dyn_dyn EverCrypt___proj__AES128_BCRYPT__item__st(EverCrypt_aes128_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES128_BCRYPT)
  {
    return projectee.val.case_AES128_BCRYPT;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
uint8_t *EverCrypt___proj__AES128_VALE__item__w(EverCrypt_aes128_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES128_VALE)
  {
    return projectee.val.case_AES128_VALE.w;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
uint8_t *EverCrypt___proj__AES128_VALE__item__sbox(EverCrypt_aes128_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES128_VALE)
  {
    return projectee.val.case_AES128_VALE.sbox;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
uint8_t *EverCrypt___proj__AES128_HACL__item__w(EverCrypt_aes128_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES128_HACL)
  {
    return projectee.val.case_AES128_HACL.w;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
uint8_t *EverCrypt___proj__AES128_HACL__item__sbox(EverCrypt_aes128_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES128_HACL)
  {
    return projectee.val.case_AES128_HACL.sbox;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
EverCrypt_aes128_key_s *EverCrypt_aes128_create(uint8_t *k1)
{
  ...
  }
  {
    ...
    if (ite)
    {
      ...
      {
        EverCrypt_aes128_key_s lit;
        lit.tag = EverCrypt_AES128_VALE;
        lit.val.case_AES128_VALE.w = w1;
        lit.val.case_AES128_VALE.sbox = sbox;
        st = lit;
      }
    }
    else if (EverCrypt_AutoConfig2_wants_hacl())
    {
      ...
      {
        EverCrypt_aes128_key_s lit;
        lit.tag = EverCrypt_AES128_HACL;
        lit.val.case_AES128_HACL.w = w1;
        lit.val.case_AES128_HACL.sbox = sbox;
        st = lit;
      }
    }
    else
...
void EverCrypt_aes128_compute(EverCrypt_aes128_key_s *k1, uint8_t *plain, uint8_t *cipher1)
{
  ...
  if (true && EverCrypt_uu___is_AES128_VALE(k2))
  {
    ...
    {
      uint8_t *sbox = k2.val.case_AES128_VALE.sbox;
      uint8_t *w1 = k2.val.case_AES128_VALE.w;
  ...
  else if (true && EverCrypt_uu___is_AES128_HACL(k2))
  {
    ...
    {
      uint8_t *sbox = k2.val.case_AES128_HACL.sbox;
      uint8_t *w1 = k2.val.case_AES128_HACL.w;
...
void EverCrypt_aes128_free(EverCrypt_aes128_key_s *pk)
{
  ...
  if (true && EverCrypt_uu___is_AES128_VALE(k1))
  {
    ...
    {
      uint8_t *sbox = k1.val.case_AES128_VALE.sbox;
      uint8_t *w1 = k1.val.case_AES128_VALE.w;
      KRML_HOST_FREE(w1);
      KRML_HOST_FREE(sbox);
    }
    else
  ...
  else if (true && EverCrypt_uu___is_AES128_HACL(k1))
  {
    ...
    {
      uint8_t *sbox = k1.val.case_AES128_HACL.sbox;
      uint8_t *w1 = k1.val.case_AES128_HACL.w;
      KRML_HOST_FREE(w1);
      KRML_HOST_FREE(sbox);
    }
    else
...
FStar_Dyn_dyn EverCrypt___proj__AES256_OPENSSL__item__st(EverCrypt_aes256_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES256_OPENSSL)
  {
    return projectee.val.case_AES256_OPENSSL;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
FStar_Dyn_dyn EverCrypt___proj__AES256_BCRYPT__item__st(EverCrypt_aes256_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES256_BCRYPT)
  {
    return projectee.val.case_AES256_BCRYPT;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
uint8_t *EverCrypt___proj__AES256_HACL__item__w(EverCrypt_aes256_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES256_HACL)
  {
    return projectee.val.case_AES256_HACL.w;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
uint8_t *EverCrypt___proj__AES256_HACL__item__sbox(EverCrypt_aes256_key_s projectee)
{
  if (projectee.tag == EverCrypt_AES256_HACL)
  {
    return projectee.val.case_AES256_HACL.sbox;
  }
  else
  {
    KRML_HOST_PRINTF("KreMLin abort at %s:%d\n%s\n",
...
EverCrypt_aes256_key_s *EverCrypt_aes256_create(uint8_t *k1)
{
  ...
  if (EverCrypt_AutoConfig2_wants_hacl())
  {
    ...
    {
      EverCrypt_aes256_key_s lit;
      lit.tag = EverCrypt_AES256_HACL;
      lit.val.case_AES256_HACL.w = w1;
      lit.val.case_AES256_HACL.sbox = sbox;
      st = lit;
    }
  }
  else
...
void EverCrypt_aes256_compute(EverCrypt_aes256_key_s *k1, uint8_t *plain, uint8_t *cipher1)
{
  ...
  if (true && EverCrypt_uu___is_AES256_HACL(k2))
  {
    ...
    {
      uint8_t *sbox = k2.val.case_AES256_HACL.sbox;
      uint8_t *w1 = k2.val.case_AES256_HACL.w;
...
void EverCrypt_aes256_free(EverCrypt_aes256_key_s *pk)
{
  ...
  if (true && EverCrypt_uu___is_AES256_HACL(k1))
  {
    ...
    {
      uint8_t *sbox = k1.val.case_AES256_HACL.sbox;
      uint8_t *w1 = k1.val.case_AES256_HACL.w;
      KRML_HOST_FREE(w1);
      KRML_HOST_FREE(sbox);
    }
    else
      ...
      KRML_CHECK_SIZE(sizeof (uint8_t), (len + (uint32_t)15U) / (uint32_t)16U * (uint32_t)16U);
      {
        ...
        KRML_CHECK_SIZE(sizeof (uint8_t), (len + (uint32_t)15U) / (uint32_t)16U * (uint32_t)16U);
        {
          ...
          KRML_CHECK_SIZE(sizeof (uint8_t),
            (adlen + (uint32_t)15U) / (uint32_t)16U * (uint32_t)16U);
          {
            uint8_t ad_[(adlen + (uint32_t)15U) / (uint32_t)16U * (uint32_t)16U];
            memset(ad_,
              0U,
              (adlen + (uint32_t)15U) / (uint32_t)16U * (uint32_t)16U * sizeof ad_[0U]);
            ...
            {
              gcm_args lit;
              lit.plain = plaintext_;
              lit.plain_len = (uint64_t)len;
              lit.aad = ad_;
              lit.aad_len = (uint64_t)adlen;
              lit.iv = iv_;
              lit.expanded_key = expanded;
              lit.cipher = cipher_;
              lit.tag = tag;
              {
                gcm_args b = lit;
    ...
    case EverCrypt_AES128_GCM:
      {
        ...
        if (ite)
        {
          ...
          {
            EverCrypt__aead_state lit;
            lit.tag = EverCrypt_AEAD_AES128_GCM_VALE;
            lit.val.case_AEAD_AES128_GCM_VALE = xk;
            st = lit;
          }
        }
        else if (EverCrypt_AutoConfig2_wants_openssl())
        {
          EverCrypt__aead_state lit;
          lit.tag = EverCrypt_AEAD_OPENSSL;
          lit.val.case_AEAD_OPENSSL =
            EverCrypt_OpenSSL_aead_create(EverCrypt_OpenSSL_AES128_GCM,
              k1);
          st = lit;
        }
        else
    ...
    case EverCrypt_AES256_GCM:
      {
        ...
        if (ite)
        {
          ...
          {
            EverCrypt__aead_state lit;
            lit.tag = EverCrypt_AEAD_AES256_GCM_VALE;
            lit.val.case_AEAD_AES256_GCM_VALE = xk;
            st = lit;
          }
        }
        else if (EverCrypt_AutoConfig2_wants_openssl())
        {
          ...
          lit.tag = EverCrypt_AEAD_OPENSSL;
          lit.val.case_AEAD_OPENSSL =
            EverCrypt_OpenSSL_aead_create(EverCrypt_OpenSSL_AES256_GCM,
              k1);
          st = lit;
        }
        else
    ...
    case EverCrypt_CHACHA20_POLY1305:
      {
        if (EverCrypt_AutoConfig2_wants_hacl())
        {
          ...
          {
            EverCrypt__aead_state lit;
            lit.tag = EverCrypt_AEAD_CHACHA20_POLY1305_HACL;
            lit.val.case_AEAD_CHACHA20_POLY1305_HACL = k01;
            st = lit;
          }
        }
        else if (EverCrypt_AutoConfig2_wants_openssl())
        {
          ...
          lit.tag = EverCrypt_AEAD_OPENSSL;
          lit.val.case_AEAD_OPENSSL =
            EverCrypt_OpenSSL_aead_create(EverCrypt_OpenSSL_CHACHA20_POLY1305,
              k1);
          st = lit;
        }
        else
...
)
{
  ...
  if (true && EverCrypt_uu___is_AEAD_AES128_GCM_VALE(k1))
  {
    ...
    {
      xk = k1.val.case_AEAD_AES128_GCM_VALE;
    }
    else
    {
  ...
  else if (true && EverCrypt_uu___is_AEAD_AES256_GCM_VALE(k1))
  {
    ...
    {
      xk = k1.val.case_AEAD_AES256_GCM_VALE;
    }
    else
    {
      xk = KRML_EABORT(uint8_t *, "unreachable (pattern matches are exhaustive in F*)");
  ...
  else if (true && EverCrypt_uu___is_AEAD_CHACHA20_POLY1305_HACL(k1))
  {
    ...
    {
      key = k1.val.case_AEAD_CHACHA20_POLY1305_HACL;
    }
    else
    {
...
void EverCrypt_aead_free(EverCrypt__aead_state *pk)
{
  ...
  if (true && EverCrypt_uu___is_AEAD_AES128_GCM_VALE(k1))
  {
    if (k1.tag == EverCrypt_AEAD_AES128_GCM_VALE)
    {
      uint8_t *xk = k1.val.case_AEAD_AES128_GCM_VALE;
      KRML_HOST_FREE(xk);
    }
    else
    {
  ...
  else if (true && EverCrypt_uu___is_AEAD_AES256_GCM_VALE(k1))
  {
    if (k1.tag == EverCrypt_AEAD_AES256_GCM_VALE)
    {
      uint8_t *xk = k1.val.case_AEAD_AES256_GCM_VALE;
      KRML_HOST_FREE(xk);
    }
    else
    {
  ...
  else if (true && EverCrypt_uu___is_AEAD_CHACHA20_POLY1305_HACL(k1))
  {
    if (k1.tag == EverCrypt_AEAD_CHACHA20_POLY1305_HACL)
    {
      uint8_t *key = k1.val.case_AEAD_CHACHA20_POLY1305_HACL;
      KRML_HOST_FREE(key);
    }
    else
    {
...
)
{
  ...
  if (EverCrypt_AutoConfig2_wants_openssl())
  {
    EverCrypt__dh_state lit;
    lit.tag = EverCrypt_DH_OPENSSL;
    lit.st = EverCrypt_OpenSSL_dh_load_group(dh_p, dh_p_len, dh_g, dh_g_len, dh_q, dh_q_len);
    st = lit;
  }
  else
  {
...
EverCrypt__ecdh_state *EverCrypt_ecdh_load_curve(EverCrypt_ec_curve g1)
{
  ...
  if (EverCrypt_AutoConfig2_wants_openssl())
  {
    ...
    {
      EverCrypt__ecdh_state lit;
      lit.tag = EverCrypt_ECDH_OPENSSL;
      lit.st = EverCrypt_OpenSSL_ecdh_load_curve(g_);
      st = lit;
    }
  }
  else
