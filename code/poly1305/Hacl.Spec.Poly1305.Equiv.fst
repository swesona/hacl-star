...
val poly_update_multi_lemma1:
  ...
  Lemma
   (let rw = compute_rw #w r in
    ...
    let nb0 = len0 / size_block in
    normalize_n #w (load_acc #w acc0 t0) r ==
      Loops.repeat_right 0 nb 0 nb0 (Loops.fixed_a pfelem) repeat_bf_t0 acc0)
let poly_update_multi_lemma1 #w text acc0 r =
  ...
  assert (normalize_n #w acc1 r == Loops.repeati nb0 (len0 / size_block) repeat_bf_s0 acc0);
  ...
  let rec aux (n:nat{n <= len0 / size_block}) : Lemma
    (Loops.repeati nb0 n repeat_bf_s0 acc0 == Loops.repeati nb n repeat_bf_t0 acc0) =
    if n = 0 then (
      Loops.eq_repeati0 nb0 repeat_bf_s0 acc0;
      Loops.eq_repeati0 nb repeat_bf_t0 acc0
    ) else (
      Loops.unfold_repeati nb0 n repeat_bf_s0 acc0 (n-1);
      Loops.unfold_repeati nb n repeat_bf_t0 acc0 (n-1);
      aux (n-1);
      let next_p = Loops.repeati nb0 (n-1) repeat_bf_s0 acc0 in
      let next_v = Loops.repeati nb (n-1) repeat_bf_t0 acc0 in
      assert (next_p == next_v);
      aux_repeat_bf (n-1) next_p;
      assert (repeat_bf_s0 (n-1) next_p == repeat_bf_t0 (n-1) next_v)
    )
  in aux (len0 / size_block);
  assert (normalize_n #w acc1 r == Loops.repeati nb (len0 / size_block) repeat_bf_t0 acc0);
  Loops.repeati_def nb (len0 / size_block) repeat_bf_t0 acc0
  ...
  Lemma
   (let rw = compute_rw #w r in
    ...
    normalize_n #w acc2 r ==
      Loops.repeat_right 0 nb (len0 / size_block) (len / size_block)
...
let poly_update_multi_lemma2 #w text acc0 r =
  ...
  assert (acc3 == Loops.repeati nb1 (len1 / size_block) repeat_bf_s1 (normalize_n #w acc1 r));
  let acc1_s = normalize_n #w acc1 r in

  Loops.repeati_def nb1 (len1 / size_block) repeat_bf_s1 acc1_s;
  assert (acc3 ==
    Loops.repeat_right 0 nb1 0 (len1 / size_block)
    (Loops.fixed_a pfelem)
    repeat_bf_s1 acc1_s);

  let aux_repeat_bf (i:nat{i < nb1}) (acc:pfelem) : Lemma
    (repeat_bf_s1 i acc == repeat_bf_t1 (nb0 + i) acc)
    =
      let start = nb0 * size_block in
  ...
  let rec aux (n:nat{n <= nb1}) : Lemma
    (Loops.repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1_s ==
     Loops.repeat_right 0 nb nb0 (nb0+n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1_s) =
    if n = 0 then (
      Loops.eq_repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1_s;
      Loops.eq_repeat_right 0 nb nb0 (nb0+n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1_s
    ) else (
      Loops.unfold_repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1_s (n-1);
      Loops.unfold_repeat_right 0 nb nb0 (nb0+n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1_s (nb0+n-1);
      aux (n-1);
      let next_p = Loops.repeat_right 0 nb1 0 (n-1) (Loops.fixed_a pfelem) repeat_bf_s1 acc1_s in
      let next_v = Loops.repeat_right 0 nb nb0 (nb0+n-1) (Loops.fixed_a pfelem) repeat_bf_t1 acc1_s in
      assert (nb0+n-1 == nb0+(n-1));
      assert (next_p == next_v);
      aux_repeat_bf (n-1) next_p;
      assert (repeat_bf_s1 (n-1) next_p == repeat_bf_t1 (nb0+n-1) next_v)
    )
  in aux nb1;
  assert (acc3 ==
    Loops.repeat_right 0 nb (len0 / size_block) (len / size_block)
...
let poly_update_multi_lemma #w text acc0 r =
  ...
  assert (normalize_n #w acc1 r ==
    Loops.repeat_right 0 nb 0 (len0 / size_block) (Loops.fixed_a pfelem) repeat_bf_t0 acc0);
  ...
  Loops.repeat_right_plus 0 nb
    0 (len0 / size_block) (len / size_block)
    (Loops.fixed_a pfelem)
    repeat_bf_t0 acc0;
  assert (acc3 ==
    Loops.repeat_right 0 nb 0 (len / size_block)
    (Loops.fixed_a pfelem)
    repeat_bf_t0 acc0);
  Loops.repeati_def nb (len / size_block) repeat_bf_t0 acc0;
...
val poly_eq_lemma1:
  ...
  (ensures (
    ...
    let nb = len / size_block in
    let nb0 = len0 / size_block in
    Loops.repeati nb0 (len0 / size_block) repeat_bf_s0 acc0 ==
      Loops.repeat_right 0 nb 0 (len0 / size_block) (Loops.fixed_a pfelem) repeat_bf_t0 acc0))
let poly_eq_lemma1 #w text acc0 r =
  ...
  lemma_repeat_blocks_multi #uint8 #pfelem size_block t0 (update1 r size_block) acc0;
  assert (acc1 == Loops.repeati nb0 (len0 / size_block) repeat_bf_s0 acc0);

  let aux_repeat_bf (i:nat{i < len0 / size_block}) (acc:pfelem) : Lemma
    (repeat_bf_s0 i acc == repeat_bf_t0 i acc)
    =
      assert ((i+1) * size_block <= nb0 * size_block);
      let block = Seq.slice text (i*size_block) (i*size_block+size_block) in
      FStar.Seq.lemma_len_slice text (i*size_block) (i*size_block+size_block);
      assert (repeat_bf_s0 i acc == f block acc);
      assert (repeat_bf_t0 i acc == f block acc)
  ...
  in aux (len0 / size_block);
  assert (acc1 == Loops.repeati nb (len0 / size_block) repeat_bf_t0 acc0);
  Loops.repeati_def nb (len0 / size_block) repeat_bf_t0 acc0
#pop-options
...
val poly_eq_lemma2:
  ...
  (ensures (
    ...
    Loops.repeati nb1 (len1 / size_block) repeat_bf_s1 acc1 ==
      Loops.repeat_right 0 nb (len0 / size_block) (len / size_block) (Loops.fixed_a pfelem) repeat_bf_t1 acc1))
...
let poly_eq_lemma2 #w text acc0 r =
  ...
  let acc1 = poly_update_multi #w t0 acc0 r in
  let acc3 = Loops.repeati nb1 (len1 / size_block) repeat_bf_s1 acc1 in
  Loops.repeati_def nb1 (len1 / size_block) repeat_bf_s1 acc1;
  assert (acc3 ==
    Loops.repeat_right 0 nb1 0 (len1 / size_block)
    (Loops.fixed_a pfelem)
    repeat_bf_s1 acc1);
  ...
  let aux_repeat_bf (i:nat{i < nb1}) (acc:pfelem) : Lemma
    ...
    =
      ...
      FStar.Seq.lemma_len_slice text (start+i*size_block) (start+i*size_block+size_block);
      assert (repeat_bf_s1 i acc == f block acc);
      assert (repeat_bf_t1 (nb0 + i) acc == f block acc)
  in

  let rec aux (n:nat{n <= nb1}) : Lemma
    (Loops.repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1 ==
     Loops.repeat_right 0 nb nb0 (nb0 + n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1) =
    if n = 0 then (
      Loops.eq_repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1;
      Loops.eq_repeat_right 0 nb nb0 (nb0+n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1
    ) else (
      let lp = Loops.repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1 in
      let rp = Loops.repeat_right 0 nb nb0 (nb0 + n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1 in
      Loops.unfold_repeat_right 0 nb1 0 n (Loops.fixed_a pfelem) repeat_bf_s1 acc1 (n-1);
      Loops.unfold_repeat_right 0 nb nb0 (nb0+n) (Loops.fixed_a pfelem) repeat_bf_t1 acc1 (nb0+n-1);
      let next_p = Loops.repeat_right 0 nb1 0 (n-1) (Loops.fixed_a pfelem) repeat_bf_s1 acc1 in
      let next_v = Loops.repeat_right 0 nb nb0 (nb0+n-1) (Loops.fixed_a pfelem) repeat_bf_t1 acc1 in
      assert (lp == repeat_bf_s1 (n-1) next_p);
      assert (rp == repeat_bf_t1 (nb0+n-1) next_v);
      aux (n-1);
      assert (next_p == next_v);
      aux_repeat_bf (n-1) next_p;
      assert (repeat_bf_s1 (n-1) next_p == repeat_bf_t1 (nb0+n-1) next_v)
...
val poly_eq_lemma12:
  ...
  (ensures (
    ...
    let acc1 = poly_update_multi #w (Seq.slice text 0 len0) acc0 r in
    Loops.repeati nb1 (len1 / size_block) repeat_bf_s1 acc1 ==
      Loops.repeati nb (len / size_block) repeat_bf_t1 acc0))
let poly_eq_lemma12 #w text acc0 r =
  ...
  assert (acc1 ==
    Loops.repeat_right 0 nb 0 (len0 / size_block)
  ...
  assert (acc3 ==
    ...
    (Loops.fixed_a pfelem)
    repeat_bf_t1 acc0);
  Loops.repeati_def nb (len / size_block) repeat_bf_t1 acc0
...
let poly_eq_lemma_pos #w text acc0 r =
  ...
  assert (acc3 == Loops.repeati nb (len / size_block) repeat_bf_t1 acc0);
...
let poly_eq_lemma_vec #w text acc r =
  ...
  let acc_s = Loops.repeati nb nb repeat_bf_s acc in
  let acc_v = Loops.repeati nb nb repeat_bf_v acc in
  ...
  let rec aux (n:nat{n <= nb}) : Lemma
    (Loops.repeati nb n repeat_bf_s acc == Loops.repeati nb n repeat_bf_v acc) =
    if n = 0 then (
      Loops.eq_repeati0 nb repeat_bf_s acc;
      Loops.eq_repeati0 nb repeat_bf_v acc
    ) else (
      let lp = Loops.repeati nb n repeat_bf_s acc in
      let rp = Loops.repeati nb n repeat_bf_v acc in
      Loops.unfold_repeati nb n repeat_bf_s acc (n-1);
      let next_s = Loops.repeati nb (n-1) repeat_bf_s acc in
      assert (lp == repeat_bf_s (n-1) next_s);
      Loops.unfold_repeati nb n repeat_bf_v acc (n-1);
      let next_v = Loops.repeati nb (n-1) repeat_bf_v acc in
...
let lemma_repeat_blocks_exact_size r b acc =
  ...
  let acc1 = Lib.LoopCombinators.repeati 1 1 (repeat_blocks_f 16 b f 1) acc in
  Lib.LoopCombinators.unfold_repeati 1 1 (repeat_blocks_f 16 b f 1) acc 0;
...
let lemma_repeat_blocks_small_size r len b acc =
  ...
  let acc1 = Lib.LoopCombinators.repeati 0 0 (repeat_blocks_f 16 b f 0) acc in
